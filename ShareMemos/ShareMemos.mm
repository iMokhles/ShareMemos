...
static NSBundle* getBundle() {
    return [NSBundle bundleWithPath:@"/Library/PreferenceBundles/ShareMemos.bundle"];
}
...
#define tweak_defaults @"/User/Library/Preferences/com.imokhles.ShareMemos.plist"
...
@interface ShareMemosBannerCell : PSTableCell {
    UIImageView *_imageView;
}
@end

@interface ShareMemosListController: PSListController {
}
@end

@implementation ShareMemosListController
- (id)specifiers {
	if(_specifiers == nil) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"ShareMemos" target:self] retain];
	}
	return _specifiers;
}
-(void)loadView {
...
-(void)shareTapped:(UIBarButtonItem *)sender {
    ...
    NSString *language = [[[NSBundle mainBundle] preferredLocalizations] objectAtIndex:0];
    if ([language isEqualToString:@"ar"]) {
        text = @"استخدم اداة #ShareMemos بواسطة @iMokhles لرفع اي ملف من تطبيق Safari";
    } else {
        text = @"I like #ShareMemos by @iMokhles to upload any file from Safari App.";
    }    
...
- (NSString *)versionValue:(PSSpecifier *)specifier {
    ...
    [task setArguments:[NSArray arrayWithObjects: @"-c", @"dpkg -s com.iosdevt.ShareMemos | grep 'Version'", nil]];
...
@implementation ShareMemosBannerCell
- (id)initWithSpecifier:(PSSpecifier *)specifier
{
    self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"headerCell" specifier:specifier];
    if (self) {
        [self setBackgroundColor:[UIColor colorWithRed:0.369f green:0.169f blue:0.361f alpha:1.0f]];
        _imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[getBundle() pathForResource:@"ShareMemosHeader" ofType:@"png"]]];
        // frame.origin.y = 100;
        if (IS_IPAD) {
            _imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[getBundle() pathForResource:@"ShareMemosHeader@2x" ofType:@"png"]]];
            _imageView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, 384);
        } else {
            _imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[getBundle() pathForResource:@"ShareMemosHeader@2x" ofType:@"png"]]];
            _imageView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, 192);
        }
